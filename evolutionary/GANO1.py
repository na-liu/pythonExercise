...
def evoltution(generation):
    ...
    print('结果：', round(float(proportion[re[0]]), 4))
    # ---------------------------------------------------
    select = roulette(proportion, N)  # 轮盘赌选择的个体m个
    ...
    the_generation = generation + offspring1
    ...
    if len(offspring1) == 0:
        ...
        # ------------
        re1 = list(map(fit.index, heapq.nlargest(int(N / 5), fit)))  # 选出适应度最大的E个个体，放到下一代
        next_generation = []
        for i in range(len(re1)):
            next_generation.append(the_generation[re1[i]])

        select1 = roulette(prop, N - int(N / 5))  # 轮盘赌选择，选出N - E个个体，放入下一代
...
if __name__ == '__main__':
    ...
    generation = init_generation(N)  # 获取第一代种群
    for i in range(200):  # 进化200代
        generation = evoltution(generation)
